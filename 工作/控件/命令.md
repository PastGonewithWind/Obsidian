...
//前缀支持常量${C_BY} ${C_GRP} ${C_TENANT} 和日期${DATE(yyyyMMdd)} 日期pattern是标准java里的simpleDateFormat的pattern  yyyyMMddHHmmss
var id= await getIncrement('IVES  ', guid())
out(id)
```

groovy
```groovy
//groovy最开始加上这个 debug才会打出日志
package com.zo.groovy.scripts
```